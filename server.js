...
//mongo db
const mongoURI =
    "mongodb+srv://admin-akshay:Test123@cluster0.icp4w.mongodb.net/IETERecruitmentPortal";
...
app.get("/instruction/:domain", async function (req, res) {
    ...
    if (auth && req.user.verified) {
        ...
        if (num == 0) {
            ...
            console.log(utils);
            let timings = "10";//utils[0].timings[req.params.domain];
            let totalQuestions = 10;//utils[0].totalQuestions[req.params.domain];
    ...
    if (nowDate >= startTime && nowDate < endTime) {
        if (auth && req.user.verified) {
            ...
            if (num == 0) {
                ...
                let domain = req.params.domain;
                let utils = await Utils.find();
                let timings = "10"//utils[0].timings[domain];
                if (domain === "ece") {
        ...
        let message = encodeURIComponent(
            `The Recruitment Test has ended at ${enddate}.`
        ...
        let message = encodeURIComponent(
            `The Recruitment Test will begin at ${startdate}...`
        );
        res.redirect("/?message=" + message);
    }
});
...
// });
app.use('/admin', admin);
app.use('/admin/questionSubmit', admin);
